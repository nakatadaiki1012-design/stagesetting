...
(function (SS) {
  ...
  function renderPartBar() {
    ...
    partBarKey = key;
    // 部が1つのときは何も出さない（「もっと…」→「1部・2部・3部」から作る）。2部以上を作った人には、部のタブを出す
    if (!d.parts) { bar.innerHTML = ''; return; }
  ...
  $('moreBlocks').onclick = () => { showTabMore(false); closePanels(); setBlocks(true); };
  ...
  function showHelp() {
    openModal(`
      ...
      <h3 style="font-size:14px">上級機能（左のタブの「もっと…」の中）</h3>
      <p class="hint small">使わなくても配置図は作れます。必要になったら使ってください。</p>
      <ul>
        ...
        <li><b>📑 1部・2部・3部</b>：「もっと…」→ <b>「📑 1部・2部・3部」</b>で、演奏会の部ごとの配置図を作れます（「いまの部をコピーして次の部を作る」がおすすめ）。2部以上を作ると、舞台図の左下に部のタブが出て、押すと切りかわります。開いている部のタブか「＋」で名前・順番・消す。</li>
        <li><b>🔁 転換の計画</b>：部のタブの <b>「🔁 転換」</b>を押すと、前の部から次の部へ変えるとき、<b>いすを何脚はけるか・何を出すか・何を動かすか</b>の表と、おすすめの手順（①はける ②ひな壇の組み替え ③動かす ④出す、出入り口・そでごと）が出ます。「図で見る」で次の部の上に × はける・○ 出す・→ 動かす の印を出し、直すとすぐ変わります。「文字でコピー」で係の人に送れます。</li>
        <li><b>🧩 パートのかたまり</b>：「もっと…」→ <b>「🧩 パートのかたまりで動かす」</b>で、パートごとのかたまりの表示になります。かたまりをドラッグするとパートごと動き、<b>ほかのパートの上で離すと並び順を入れかえ</b>ます（吹奏楽・ブラスバンドのかんたん編成では、ひな壇・間隔をそろえたまま並べ直し、並び方は「自分で並べかえた順」になります）。終わるときは、下の帯の「終わる」か、右の 🧩 を押します。</li>
      </ul>
